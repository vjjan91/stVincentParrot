...
# load elevation
alt <- terra::rast("data/elevation/alt")
...
At this stage, we work with Forestry agents from SVG to determine what points should be ultimately chosen for the monitoring of the St. Vincent's parrot
```{r}
...
surveyAttr <- read.csv("data/surveyPointAttributes.csv")
...
data <- bind_rows(watchPoints[,c(2,3,4)],
                  gapPoints[,c(11,12,13)],
                  potentialAM[,c(9,10,11)],
                  surveyDesign[,c(6,7,8)])

trial <- as.data.frame(st_distance(data))
```
